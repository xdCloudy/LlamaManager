...
pub enum ProfileGenerationError {
    ...
    #[error(
        "profile section [{0}] already exists; generator will not overwrite an existing section"
    )]
    ...
    #[error(
        "selected model/runtime pairing is {status}; a safe starting profile cannot be generated"
    )]
    ...
    for required in request.required_settings {
        ...
        if required.explicitly_disabled {
            unresolved.push(UnresolvedProfileChoice {
                key: required.key.clone(),
                reason:
                    "required feature was explicitly disabled by the user; setting was not emitted"
                        .into(),
            });
            continue;
        }
...
fn add_projector_setting(
    ...
    let multimodal_disabled = request.explicitly_disabled_features.contains("multimodal");
...
mod tests {
    ...
    fn emits_only_explicit_evidence_backed_supported_recommendations() {
        ...
        assert!(
            generated
                .source
                .contains("model=D:\\Models 外部\\my model.gguf\r\n")
        );
    ...
    fn unsupported_required_feature_is_hard_failure_unless_explicitly_disabled() {
        ...
        let generated =
            generate_profile(&request(&runtime, &selected_model, &compat, &[], &disabled)).unwrap();
        assert!(!generated.source.contains("spec-type"));
        assert!(
            generated
                .unresolved
                .iter()
                .any(|item| item.key == "spec-type")
        );
    }
    ...
    fn required_multimodal_projector_is_never_silently_dropped() {
        ...
        assert!(
            generated
                .source
                .contains("mmproj=D:\\Models 外部\\视觉 projector.gguf")
        );
        assert_eq!(
    ...
    fn destination_and_preapply_diff_are_retained_without_writing() {
        ...
        let mut req = request(&runtime, &selected_model, &compat, &[], &[]);
        req.destination =
            ProfileDestination::External(PathBuf::from(r"D:\User Configs 外部\models.ini"));
        let generated = generate_profile(&req).unwrap();
