...
use crate::{
    ...
    server_process::{
        GracefulStopOutcome, ManagedProcessState, ProcessExitEvidence, ServerProcessSupervisor,
    },
    server_readiness::{
            ...
            Some(paths) => match Database::open(paths.database.clone()) {
                Ok(db) => match (db.latest_installation(), db.latest_model()) {
                    ...
                    (installation, model) => {
                        ...
                        notice = Some((false, message));
                        (None, None)
                    }
                },
                Err(error) => {
                    notice = Some((false, format!("Could not open application database: {error}")));
                    (None, None)
                }
            },
            None => (None, None),
        ...
        match check_port_available(&ServerEndpoint::loopback(8080)) {
            Ok(PortAvailability::InUse) => {
                ...
                if notice.is_none() {
                    notice = Some((
                        false,
                        "127.0.0.1:8080 is already occupied. It is shown as UNKNOWN/unowned rather than assumed to be a managed LlamaWave server. Choose another port or stop the external process."
                            .into(),
                    ));
                }
            }
            ...
            Err(error) => {
                if notice.is_none() {
                    notice = Some((false, format!("Could not reconcile default server port: {error}")));
                }
            }
        }
...
fn prepare_launch(state: &ServerUiState) -> Result<PreparedLaunch, String> {
    let installation = state
        .installation
        .as_ref()
        .ok_or_else(|| "No persisted llama.cpp installation is selected. Select one in CORE LAB first.".to_string())?;
    ...
    let model = state
        .model
        .as_ref()
        .ok_or_else(|| "No persisted GGUF model is selected. Add/select one in MODEL LIBRARY or CORE LAB first.".to_string())?;
    ...
    let spec = build_server_launch_spec(installation, &settings).map_err(|error| error.to_string())?;
    ...
    Ok(PreparedLaunch {
        ...
        endpoint,
        paths,
        secrets: (!api_key.is_empty()).then_some(api_key).into_iter().collect(),
    })
}

fn refresh_sources(mut state: Signal<ServerUiState>) {
    ...
    thread::spawn(move || {
        let result = Database::open(paths.database.clone()).and_then(|db| {
            Ok((db.latest_installation()?, db.latest_model()?))
        });
        ...
        match result {
            Ok((installation, model)) => {
                ...
                current.model = model;
                current.notice = Some((true, "Reloaded persisted runtime and model evidence.".into()));
            }
...
fn run_start(mut state: Signal<ServerUiState>) {
    let snapshot = state.read().clone();
...

    {
        ...
        current.notice = Some((true, "Validating launch command and endpoint before spawn…".into()));
    ...
    let prepared = match prepare_launch(&snapshot) {
        ...
        Err(error) => {
            ...
            current.lifecycle.mark_failed(error.clone());
    ...
    let logs = match ServerLogCapture::with_disk(
        ...
        Err(error) => {
            ...
            current.operation = ServerOperation::Idle;
            current.lifecycle.mark_failed(error.to_string());
            current.notice = Some((false, format!("Could not initialize bounded server log capture: {error}")));
            return;
        }
    };
    ...
    let identity = {
        let mut supervisor = match supervisor.lock() {
            ...
            Err(_) => {
                let mut current = state.write();
                current.operation = ServerOperation::Idle;
                current.lifecycle.mark_failed("Server process supervisor mutex was poisoned.");
                current.notice = Some((false, "Server process supervisor mutex was poisoned.".into()));
                return;
            }
        };
    ...
    let readiness = {
        let mut supervisor = match supervisor.lock() {
            ...
            Err(_) => {
                let mut current = state.write();
                current.operation = ServerOperation::Idle;
                current.lifecycle.mark_failed("Server process supervisor mutex was poisoned during readiness.");
                current.notice = Some((false, "Server process supervisor mutex was poisoned during readiness.".into()));
                return;
            }
        };
    ...
    match readiness {
        ...
        Err(ServerReadinessError::ProcessExited { evidence }) => {
            ...
            current.notice = Some((false, format!("llama-server exited before readiness: {evidence:?}")));
        }
        Err(ServerReadinessError::Cancelled) => {
            current.notice = Some((
                true,
                "Readiness polling was cancelled. Waiting for the requested stop operation to reconcile the process.".into(),
            ));
        }
        Err(error) => {
            current.lifecycle.mark_failed(error.to_string());
            current.notice = Some((
                false,
                format!("Server process exists but readiness/inference failed: {error}. Inspect logs, then STOP or FORCE KILL."),
            ));
        }
    }
}

fn spawn_log_pump(mut state: Signal<ServerUiState>, logs: ServerLogCapture, generation: u64) {
    thread::spawn(move || loop {
        ...
        state.write().logs = Some(snapshot);
        if !keep_running {
            break;
        }
...
fn spawn_process_monitor(
    mut state: Signal<ServerUiState>,
    supervisor: Arc<Mutex<ServerProcessSupervisor>>,
    generation: u64,
) {
    thread::spawn(move || loop {
        ...
        let observed = match supervisor.lock() {
            ...
            Err(_) => {
                let mut current = state.write();
                current.operation = ServerOperation::Idle;
                current.lifecycle.mark_failed("Server process supervisor mutex was poisoned while monitoring.");
                current.notice = Some((false, "Server process supervisor mutex was poisoned while monitoring.".into()));
                break;
            }
        };
        ...
        match observed {
            ...
            Ok(Some(ManagedProcessState::Exited { evidence, .. })) => {
                ...
                current.process_owned = false;
                current.operation = ServerOperation::Idle;
                let was_stopping = current.lifecycle.snapshot().phase == ServerLifecyclePhase::Stopping;
                current.lifecycle.mark_exit(evidence.clone());
                if !was_stopping {
                    current.notice = Some((
                        false,
                        format!("Managed llama-server exited unexpectedly: {evidence:?}. Retained logs remain available below."),
                    ));
                }
                break;
            }
...
fn run_graceful_stop(mut state: Signal<ServerUiState>, restart: bool) {
    let snapshot = state.read().clone();
    ...
    snapshot.cancellation.store(true, Ordering::SeqCst);
    {
        ...
        current.notice = Some((
            true,
            "Requesting llama-server's Windows Ctrl+C graceful shutdown; bounded wait is 5 seconds.".into(),
        ));
    }
    ...
    let outcome = {
        let mut supervisor = match snapshot.supervisor.lock() {
            ...
            Err(_) => {
                ...
                current.notice = Some((false, "Server process supervisor mutex was poisoned; force-kill may be required.".into()));
                return;
            }
        };
        let process = match supervisor.process_mut() {
    ...
    match outcome {
        Ok(GracefulStopOutcome::Exited(evidence)) => {
            ...
            let final_logs = snapshot.active_logs.as_ref().map(ServerLogCapture::snapshot);
            {
                let mut current = state.write();
                current.process_owned = false;
                current.operation = ServerOperation::Idle;
                ...
                current.notice = Some((true, format!("Managed server stopped through graceful Ctrl+C: {evidence:?}")));
            }
            if restart {
        ...
        Ok(GracefulStopOutcome::GracePeriodExpired { waited }) => {
            ...
            if current.readiness.is_some() {
                ...
                current.lifecycle.mark_failed("Graceful stop timed out while the process remained alive.");
            }
            current.notice = Some((
                false,
                format!(
        ...
        Err(error) => {
            ...
            current.notice = Some((
                false,
                format!("Graceful stop request failed without claiming success: {error}. Confirm FORCE KILL if needed."),
            ));
        }
    }
}

fn run_force_kill(mut state: Signal<ServerUiState>) {
    let snapshot = state.read().clone();
    ...
    let result = match snapshot.supervisor.lock() {
        Ok(mut supervisor) => supervisor.process_mut().and_then(|process| process.force_kill()),
        Err(_) => {
            let mut current = state.write();
            current.operation = ServerOperation::Idle;
            current.notice = Some((false, "Server process supervisor mutex was poisoned.".into()));
    ...
    match result {
        Ok(evidence) => {
            ...
            current.process_owned = false;
            current.lifecycle.mark_exit(evidence.clone());
            current.logs = snapshot.active_logs.as_ref().map(ServerLogCapture::snapshot);
            current.notice = Some((true, format!("Managed process tree FORCE KILLED explicitly: {evidence:?}")));
        }
        Err(error) => {
            current.notice = Some((false, format!("Force kill failed: {error}")));
        }
    }
}
...
fn phase_class(phase: ServerLifecyclePhase) -> &'static str {
    match phase {
        ServerLifecyclePhase::Ready => "sv-badge",
        ServerLifecyclePhase::Starting | ServerLifecyclePhase::Stopping | ServerLifecyclePhase::Unknown => "sv-badge warn",
        ServerLifecyclePhase::Failed | ServerLifecyclePhase::Crashed => "sv-badge error",
        ServerLifecyclePhase::Stopped => "sv-badge",
    }
}
...
pub fn ServerLifecycleView() -> Element {
    ...
    let can_start = !snapshot.process_owned
        && snapshot.operation == ServerOperation::Idle
        && preview.is_ok();
    ...
    let mut visible_logs = snapshot
        .logs
        .as_ref()
        .map(|logs| logs.entries.iter().rev().take(180).cloned().collect::<Vec<_>>())
        .unwrap_or_default();
    ...
    let force = move |_| {
        if !state.read().force_confirm {
            ...
            current.notice = Some((
                false,
                "FORCE KILL terminates the Windows Job Object process tree without graceful model/server cleanup. Click CONFIRM FORCE KILL to proceed.".into(),
            ));
            return;
        }
        let worker_state = state;
    ...
    let refresh = move |_| refresh_sources(state);
    let export = move |_| {
        let snapshot = state.read().clone();
        match export_logs(&snapshot) {
            Ok(path) => state.write().notice = Some((true, format!("Exported redacted logs to {}", path.display()))),
    ...
    rsx! {
        ...
        main { class: "sv-page",
            ...
            div { class: "sv-grid",
                section { class: "sv-panel",
                    div { class: "sv-panel-head",
                        ...
                        button { class: "sv-button", disabled: snapshot.operation != ServerOperation::Idle, onclick: refresh, "REFRESH SOURCES" }
                    }
                    div { class: "sv-panel-body",
                        ...
                        div { class: "sv-path",
                            ...
                            if let Some(installation) = snapshot.installation.as_ref() {
                                if let Some(server) = installation.server.as_ref() { "{server.path.display()}" } else { "selected runtime has no llama-server" }
                            } else { "no persisted runtime selected" }
                            "\n\nMODEL\n"
                            if let Some(model) = snapshot.model.as_ref() { "{model.path.display()}" } else { "no persisted model selected" }
                        }
                        ...
                        div { class: "sv-actions",
                            button { class: "sv-button primary", disabled: !can_start, onclick: start, "START SERVER" }
                            button { class: "sv-button", disabled: !can_stop, onclick: stop,
                                if phase == ServerLifecyclePhase::Starting { "CANCEL + STOP" } else { "STOP GRACEFULLY" }
                            }
                            button { class: "sv-button magenta", disabled: !can_restart, onclick: restart, "RESTART" }
                            button {
                ...
                aside { class: "sv-panel",
                    div { class: "sv-panel-head",
                        div { div { class: "sv-kicker", "TRUTHFUL STATE" } h2 { "READINESS + EVIDENCE" } }
                    }
                    div { class: "sv-panel-body",
                        ...
                        div { class: "sv-readiness", style: "margin-top:10px",
                            if let Some(readiness) = snapshot.readiness.as_ref() {
                                div { class: "sv-evidence",
                                    ...
                                    div { "attempts: {readiness.attempts} · elapsed: {:.3}s", readiness.elapsed.as_secs_f64() }
                            ...
                            if let Some(exit) = snapshot.lifecycle.snapshot().last_exit.as_ref() {
                                div { class: "sv-evidence", strong { "LAST EXIT" } div { "code={:?} · kind={:?} · observed={}", exit.code, exit.kind, exit.observed_at_unix_ms } }
                            }
            ...
            section { class: "sv-panel sv-log-panel",
                div { class: "sv-panel-head",
                    div { div { class: "sv-kicker", "STDOUT + STDERR" } h2 { "RETAINED SERVER LOG" } }
                    button { class: "sv-button", disabled: snapshot.logs.is_none(), onclick: export, "EXPORT REDACTED LOG" }
                }
                div { class: "sv-panel-body",
                    div { class: "sv-log-toolbar",
                        div { class: "sv-muted",
                            if let Some(logs) = snapshot.logs.as_ref() {
                                "{} entries shown/retained · {} bytes · {} evicted", visible_logs.len(), logs.retained_bytes, logs.evicted_entries
                            } else { "No managed log capture yet." }
                        }
                        if let Some(path) = snapshot.log_path.as_ref() { div { class: "sv-muted", "{path.display()}" } }
                    }
                    if let Some(logs) = snapshot.logs.as_ref() {
                        if let Some(error) = logs.disk_error.as_ref() { div { class: "sv-disk-error", "DISK CAPTURE ERROR: {error}" } }
                    }
